# =============================================================================
#  main.py — SkillForge v12  |  Full UI + all visual fixes applied
...
from backend import (
    ...
    _parse_bytes, _load_semantic_bg, demand_label,
    ...
    roi_bar, weekly_plan, build_dag_data, generate_interview_questions,
    build_ics_calendar,
...
# =============================================================================
#  CSS — v12: full color system fix, layout fixes, component fixes
# =============================================================================
CSS = """
<style>
...
/* FIX: Hide the Streamlit running/status bar that shows between components */
[data-testid="stStatusWidget"],
[data-testid="stBottom"],
.stBottom,
div[class*="StatusWidget"],
div[class*="statusWidget"],
div[class*="RunningIcon"],
div[class*="runningIcon"],
[data-testid="stAppRunningIndicator"],
.element-container:has(> [data-testid="stStatusWidget"]),
/* The specific bar showing "SkillForge v11 · live · API calls" is our own footer moved inside */
/* Target Streamlit's internal version string bar */
...
/* FIX: Force DM Sans on all Streamlit native components */
...
/* FIX: unified chip style — all same border style, only colored ones use .on */
...
/* FIX: inactive chips are visually muted */
.sf-chip.off{border-color:transparent;color:var(--t4);font-size:0.6rem}

/* ── PAGE ── */
.sf-page{padding:0 32px 80px;max-width:1200px;margin:0 auto}
@media(max-width:640px){.sf-page{padding:0 16px 80px}}

/* FIX: removed dead hero zone — padding starts at 24px not 44px */
.sf-hero{padding:24px 0 24px}
...
/* ── SECONDARY / OUTLINE BUTTONS — higher specificity override ── */
div.sf-secondary > div[data-testid="stButton"] > button,
div.sf-secondary .stButton > button {
...
div.sf-secondary > div[data-testid="stButton"] > button:hover,
div.sf-secondary .stButton > button:hover {
...
/* ── TABS — nuclear override for Streamlit's internal red/pink underline ── */
...
/* Kill Streamlit's internal ::after pseudo-element underline */
...
/* Streamlit sometimes uses p tags inside tab buttons */
...
/* ── BANNER ── */
.sf-banner{background:var(--s1);border:1px solid var(--border);border-radius:14px;padding:28px 32px;margin:16px 0 16px;}
...
/* FIX: Vision badge only shown when actually used */
...
/* FIX: unified score card style — all same, accent via color var only */
...
/* ── SENIORITY WARN — FIX: inline pill, not full banner ── */
...
@media(max-width:640px){.sf-skill-grid{grid-template-columns:repeat(auto-fill,minmax(160px,1fr))}}
...
/* FIX: custom checkboxes to match dark theme */
[data-testid="stCheckbox"] > label{
  background:transparent!important;
...
[data-testid="stCheckbox"] > label > div[data-testid="stCheckboxContainer"]{
...
/* FIX: missing keywords as actionable red chips */
...
/* FIX: empty state — unified neutral style, no amber warnings for missing data */
...
.sf-empty-label{font-size:0.82rem;color:var(--t2);margin-bottom:4px;font-weight:600;font-family:var(--sans);}

/* ── TRANSFER ADVANTAGES — FIX: size variation by percentage ── */
...
[data-testid="stDownloadButton"]>button{background:var(--s2)!important;border:1px solid var(--border)!important;color:var(--t2)!important;font-family:var(--sans)!important;font-weight:500!important;font-size:0.82rem!important;margin-top:auto!important;}
...
/* ── HOW IT WORKS ── */
.sf-how{display:flex;align-items:center;gap:0;margin:0 0 20px;background:var(--s1);border:1px solid var(--border);border-radius:12px;padding:20px 24px}
...
/* FIX: styled divider arrow between steps */
.sf-how-arrow{font-size:1.2rem;color:var(--teal);opacity:0.4;padding:0 12px;flex-shrink:0}

.sf-stats-strip{display:flex;align-items:center;justify-content:center;gap:0;margin:0 0 20px;background:rgba(45,212,191,0.04);border:1px solid var(--bhi);border-radius:10px;padding:16px 24px}
...
/* FIX: footer with enough padding so content never gets covered */
...
/* FIX: bottom padding so sticky footer never covers content */
.sf-page-pad{padding-bottom:72px}
...
# =============================================================================
#  TOPBAR — FIX: Vision OCR chip conditional, chip styles consistent
# =============================================================================
def render_topbar(is_image_resume: bool = False) -> None:
    ...
    <div class="sf-top">
      ...
      <div class="sf-top-right">
        ...
        {vision_chip}
        <span class="sf-chip off">NetworkX DAG</span>
      </div>
...
def render_input() -> None:
    ...
    # FIX: sample buttons styled as secondary (outlined), not primary filled
    ...
    for col, key, dlbl in zip([pc1, pc2, pc3], SAMPLES, ["Tech Role", "Data / AI Role", "Non-Tech Role"]):
        with col:
            st.markdown('<div class="sf-secondary">', unsafe_allow_html=True)
    ...
    with left:
        ...
        with up_tab:
            rf = st.file_uploader("Resume", type=["pdf", "docx", "jpg", "jpeg", "png", "webp"], key="res_file", label_visibility="collapsed")
            if rf is not None:
                try:
                    rf.seek(0)
                    raw_bytes = rf.read()
        ...
        with paste_tab:
            if st.session_state.get("_resume_source") == "file":
                ...
                rp = st.text_area("Resume text", height=220, placeholder="Paste your resume here...", key="res_paste", label_visibility="collapsed")
                if rp and rp.strip():
    ...
    with right:
        ...
        with jpaste_tab:
            jp = st.text_area("Job description", height=220, placeholder="Paste the job description here...", key="jd_paste", label_visibility="collapsed")
            if jp and jp.strip():
    ...
    with opt2_col:
        # FIX: "Force fresh" moved to collapsible section, not main flow
    ...
    with btn_col:
        ...
        if resume_ready and jd_ready:
            ...
            st.markdown(
                f'<p style="font-family:var(--mono);font-size:0.75rem;color:var(--t3);text-align:center;padding:12px 0">Add {" and ".join(missing)} to continue</p>',
...
def render_loading() -> None:
    ...
    show_steps(0)
    result = run_analysis_with_web(resume_text, jd_text, resume_image_b64=resume_img, location=st.session_state.get("sal_location", "India"))
    ...
    if "error" in result:
        ...
        if err == "rate_limited":
            ...
            st.error("Analysis quality issue: skill extraction failed for this candidate. Please try again.")
        else:
...
# =============================================================================
#  BANNER — FIX: color system, removed telemetry, hours saved logic
# =============================================================================
def render_banner(res: dict) -> None:
    ...
    # FIX: current fit color — neutral grey, not alarming red
    ...
    # FIX: Vision badge only when actually used
    ...
    # FIX: hours saved — context-aware messaging
    hours_saved = int(im.get("hours_saved") or 0)
    roadmap_longer = im.get("roadmap_longer", False)
    if roadmap_longer:
        hours_narrative = f'<span style="color:var(--t3)">Comprehensive path for your gap level</span>'
    elif hours_saved > 0:
        hours_narrative = f'saves <strong style="color:var(--green)">~{hours_saved}h</strong> vs generic'
    else:
        hours_narrative = f'<span style="color:var(--t3)">vs 60h generic</span>'
    ...
    # FIX: hours saved metric — neutral delta color when 0
    ...
    # FIX: seniority gap as pill, not full-width banner
...
def render_tab_overview(res: dict) -> None:
    ...
    with col_flt:
        filt = st.selectbox("Filter", ["All", "Missing", "Partial", "Known", "Required only"], key="gp_filter", label_visibility="collapsed")
        ...
        for g in filtered:
            ...
            # FIX: use backend demand_label for consistent signals
            ...
            html += (
                f'<div class="sf-skill-card {s.lower()}" role="region" aria-label="{g["skill"]} skill card">'
    ...
    with col_radar:
        # FIX: radar chart now auto-switches to bar when near-zero
        st.plotly_chart(animated_radar_chart(gp), use_container_width=True, config={"displayModeBar": False}, key="radar_overview")
        ...
        if tf:
            ...
            # FIX: size variation and deduplication applied in backend
            for t in tf[:5]:
        ...
        # Peer percentile based on MARKET_DEMAND and known skills
        ...
        if total_hot > 0:
            pct = round(known_hot / total_hot * 100)
            # Rough percentile: 0% known hot → bottom 20%, 100% → top 10%
            percentile = max(10, min(90, 20 + pct * 0.7))
            st.markdown(f"""
            <div style="margin-top:16px;background:rgba(167,139,250,0.06);border:1px solid rgba(167,139,250,0.2);
                        border-radius:8px;padding:12px 16px;">
              <div style="font-family:var(--mono);font-size:0.6rem;letter-spacing:0.1em;text-transform:uppercase;
                          color:var(--purple);margin-bottom:6px">Peer percentile</div>
              <div style="font-family:var(--mono);font-size:1.6rem;font-weight:500;color:var(--t1);line-height:1">
                Top {100-int(percentile)}%
              </div>
              <div style="font-size:0.72rem;color:var(--t3);margin-top:4px">
                of {res['jd'].get('role_title','')[:28]} applicants based on in-demand skill coverage
              </div>
            </div>""", unsafe_allow_html=True)
...
# =============================================================================
#  TAB: ROADMAP — FIX: weekly plan at top, load courses at top
# =============================================================================
def render_tab_roadmap(res: dict) -> None:
    path = res["path"]

    # Progress persistence via localStorage — survive page refreshes
    candidate_key = (res.get("candidate",{}).get("name","") + res.get("jd",{}).get("role_title","")).replace(" ","_")[:40]
    storage_key   = f"sf_progress_{candidate_key}"

    import streamlit.components.v1 as _components
    # Inject localStorage read/write bridge
    _components.html(f"""
    <script>
    (function() {{
      // Read saved progress into a hidden input for Streamlit
    ...
    # Load from session state (populated by localStorage on first render)
    ...
    with hd_r:
        hpd = st.select_slider("Pace (h/day)", options=[1, 2, 4, 8], value=int(st.session_state.get("hpd", 2) or 2), key="hpd_slider")
    ...
    # FIX: Load courses button at top
    ...
    # FIX: Weekly plan shown near top as "quick view" before module list
    with st.expander("📅 Week 1 study plan — click to collapse", expanded=True):
    ...
    with mod_col:
        ...
        for phase_name, mods in phases:
            ...
            for m in mods:
                ...
                chk = st.checkbox(f"{m['title']} · {int(m.get('duration_hrs') or 0)}h · {m['level']}", value=is_done, key=f"c_{m['id']}")
                if chk:
                    completed.add(m["id"])
                else:
                ...
                st.markdown(
                    f'<div class="sf-mod {lc}{dc}" role="region" aria-label="Module {idx}: {m["title"]}">'
                ...
                if courses:
                    ...
                    with lc1:
                        st.link_button(f"{crs['icon']} {crs['title'][:50]} ({crs['platform']})", crs["url"], use_container_width=True)
                else:
                    lc1, lc2 = st.columns(2)
                    with lc1:
                        st.link_button(f"Coursera: {m['skill']}", f"https://www.coursera.org/search?query={q}", use_container_width=True)
                    with lc2:
                        st.link_button(f"YouTube: {m['skill']}", f"https://www.youtube.com/results?search_query={q}+tutorial", use_container_width=True)
    ...
    if longer:
        save_note = f"Personalized path covering your specific gap ({im['modules_count']} modules)"
    else:
        save_note = f"~{saved}h saved vs generic 60h onboarding"
    st.markdown(f"""
    ...
    # Cost-to-train vs cost-to-hire ROI card
    roadmap_hrs = int(im.get("roadmap_hours", 0))
    # Assume ₹500/hr opportunity cost for self-study in India
    train_cost_inr = roadmap_hrs * 500
    # Mid-level hire cost estimate: ~₹8-12L recruitment cost
    hire_cost_inr  = 1000000
    savings_inr    = max(0, hire_cost_inr - train_cost_inr)
    st.markdown(f"""
    <div style="background:rgba(45,212,191,0.04);border:1px solid rgba(45,212,191,0.15);
                border-radius:10px;padding:18px 24px;margin-top:12px">
      <div style="font-family:var(--mono);font-size:0.62rem;letter-spacing:0.1em;
                  text-transform:uppercase;color:var(--teal);margin-bottom:12px">
        💰 Business case: train vs hire
      </div>
      <div style="display:flex;gap:0;align-items:center">
        <div style="flex:1;text-align:center">
          <div style="font-family:var(--mono);font-size:1.4rem;font-weight:500;color:var(--t1)">
            ₹{train_cost_inr:,}
          </div>
          <div style="font-family:var(--mono);font-size:0.6rem;color:var(--t3);margin-top:3px">
            Cost to train ({roadmap_hrs}h × ₹500/hr)
          </div>
        </div>
        ...
        <div style="flex:1;text-align:center">
          ...
          <div style="font-family:var(--mono);font-size:0.6rem;color:var(--t3);margin-top:3px">
            Est. cost to hire Mid-level externally
          </div>
        </div>
        <div style="color:var(--t4);padding:0 16px;font-size:1.2rem">→</div>
        <div style="flex:1;text-align:center">
          <div style="font-family:var(--mono);font-size:1.4rem;font-weight:500;color:var(--green)">
            ₹{savings_inr:,}
          </div>
          <div style="font-family:var(--mono);font-size:0.6rem;color:var(--t3);margin-top:3px">
            Estimated savings by upskilling internally
          </div>
        </div>
      </div>
    </div>""", unsafe_allow_html=True)
...
# =============================================================================
#  TAB: RESEARCH — FIX: no raw errors, unified empty states
# =============================================================================
def render_tab_research(res: dict) -> None:
    ...
    # FIX: DuckDuckGo error is silent — no raw pip install messages shown
    ...
    with q_col:
        ...
        st.text_input("Search", placeholder='"React 2025" · "FastAPI salary Bangalore" · "Docker course"', key="search_input", label_visibility="collapsed")
    ...
    if not web_available:
        st.markdown('<div class="sf-empty-state"><div class="sf-empty-icon">🔌</div><div class="sf-empty-label">Web search unavailable</div><div>Install ddgs to enable live search: <code>pip install ddgs</code></div></div>', unsafe_allow_html=True)
    else:
        ...
        if results:
            ...
            for r in shown:
                ...
                st.markdown(f'<div class="sf-search-result"><a class="sf-search-title" href="{href}" target="_blank">{r.get("title","No title")}</a><div class="sf-search-url">{domain}</div><div class="sf-search-body">{r.get("body","")[:200]}</div></div>', unsafe_allow_html=True)
        elif do_search:
            st.markdown('<div class="sf-empty-state"><div class="sf-empty-icon">🔍</div><div class="sf-empty-label">No results found</div><div>Try rephrasing the query</div></div>', unsafe_allow_html=True)
    ...
    with sal_col:
        ...
        if sal and sal_med > 0:
            ...
            # FIX: salary empty state — fetch button first, not error first
            ...
            if st.button("Fetch salary data", key="sal_fetch", use_container_width=True):
                ...
                if _sal_new:
                    ...
                    st.markdown('<div class="sf-empty-state"><div class="sf-empty-icon">💼</div><div class="sf-empty-label">No salary data found</div><div>Try a different location or role</div></div>', unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    if gap_skills_f:
        ...
        if cached:
            ...
            st.markdown(f'<div class="sf-empty-state"><div class="sf-empty-icon">📚</div><div class="sf-empty-label">No course links found for {sel_s}</div><div>Try searching manually above</div></div>', unsafe_allow_html=True)


# =============================================================================
#  TAB: ATS & EXPORT — FIX: spacing, keyword colors, metric labels, rewrite button
# =============================================================================
def render_tab_ats_export(res: dict) -> None:
    ...
    with right:
        ...
        if kws:
            # FIX: keywords styled as actionable red chips
        ...
        # FIX: interview metric — no up arrow on negative label
        iv_label = iv.get("label", "–")
        c1.metric("Interview", f"{iv.get('score', 0)}%", iv_label)
        c2.metric("Seniority gap", f"{sm.get('gap_levels', 0)} level{'s' if sm.get('gap_levels',0)!=1 else ''}" if sm.get('gap_levels',0) > 0 else "None")
        # FIX: career estimate with context label
        c3.metric("Time to level up", f"~{cgm}mo" if cgm else "On track", "est. to reach target seniority" if cgm else "")
    ...
    st.markdown('<div style="font-family:var(--mono);font-size:0.68rem;color:var(--t3);margin-bottom:12px">ATS-optimized · existing facts only · missing keywords naturally woven in</div>', unsafe_allow_html=True)

    rtxt = st.session_state.get("resume_text", "")
    if not rtxt and st.session_state.get("resume_image"):
        ei = res.get("candidate", {})
        if ei.get("name"):
            rtxt = f"{ei.get('name','')}\n{ei.get('current_role','')}\n" + "\n".join(f"- {s['skill']} ({s['proficiency']}/10)" for s in ei.get("skills", []))
    ...
    if rtxt:
        if st.button("Generate ATS-optimized rewrite ⚡", key="gen_rw", use_container_width=True):
            ...
            # FIX: estimate before/after ATS improvement
            ...
            improvement = min(30, round((kw_count_after - kw_count_before) / kw_total * 40))
            st.session_state["rw_ats_before"] = ats_pct
            st.session_state["rw_ats_after"]  = min(100, ats_pct + improvement)

        rw = st.session_state.get("rw_result")
        if rw:
            # Before/after ATS score
            before_score = st.session_state.get("rw_ats_before", ats_pct)
            after_score  = st.session_state.get("rw_ats_after",  min(100, ats_pct + 15))
            ...
            st.download_button("Download rewritten resume", data=rw, file_name="skillforge_rewritten.txt", mime="text/plain", key="dl_rewrite")
    ...
    # Shareable link — base64-encode key result fields
    ...
    <div style="background:var(--s2);border:1px solid var(--border);border-radius:8px;
      ...
      <code style="flex:1;font-family:var(--mono);font-size:0.65rem;color:var(--teal);
                   background:var(--s1);padding:4px 10px;border-radius:4px;border:1px solid var(--border);
                   overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{share_url[:80]}…</code>
      <span style="font-family:var(--mono);font-size:0.6rem;color:var(--t4)">Copy URL to share</span>
    </div>""", unsafe_allow_html=True)

    # FIX: 4 export columns including calendar
    ...
    with ex1:
        ...
        if REPORTLAB:
            ...
            st.download_button("Download PDF", data=pdf_buf, file_name=f"skillforge_{nm}_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf", use_container_width=True, key="dl_pdf")
        else:
    ...
    with ex2:
        st.markdown('<div class="sf-export-card">', unsafe_allow_html=True)
        st.markdown('<div class="sf-export-hd">JSON export</div><div class="sf-export-sub">Complete structured result for integrations and further processing</div>', unsafe_allow_html=True)
        for k, v in export_meta:
            st.markdown(f'<div class="sf-export-row"><span class="sf-ek">{k}</span><span class="sf-ev">{v}</span></div>', unsafe_allow_html=True)
        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
        export_data = {
            ...
            "roadmap": [{"id": m["id"], "title": m["title"], "skill": m["skill"], "level": m["level"], "duration_hrs": m["duration_hrs"], "is_critical": m.get("is_critical", False), "reasoning": m.get("reasoning", "")} for m in roadmap],
            "generated_at": datetime.now().isoformat(),
        }
        st.download_button("Download JSON", data=json.dumps(export_data, indent=2, default=str), file_name=f"skillforge_{datetime.now().strftime('%Y%m%d')}.json", mime="application/json", use_container_width=True, key="dl_json")
        st.markdown('</div>', unsafe_allow_html=True)

    with ex3:
        st.markdown('<div class="sf-export-card">', unsafe_allow_html=True)
        st.markdown('<div class="sf-export-hd">CSV export</div><div class="sf-export-sub">Roadmap modules in spreadsheet format for tracking progress</div>', unsafe_allow_html=True)
        for k, v in export_meta:
            st.markdown(f'<div class="sf-export-row"><span class="sf-ek">{k}</span><span class="sf-ev">{v}</span></div>', unsafe_allow_html=True)
        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
        csv_rows = ["id,title,skill,level,domain,duration_hrs,is_critical,is_required,reasoning"]
        for m in roadmap:
            # FIX: removed trailing space before comma, use full reasoning not generic
        ...
        st.download_button("Download CSV", data="\n".join(csv_rows), file_name=f"skillforge_roadmap_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv", use_container_width=True, key="dl_csv")
        st.markdown('</div>', unsafe_allow_html=True)

    with ex4:
        st.markdown('<div class="sf-export-card">', unsafe_allow_html=True)
        st.markdown('<div class="sf-export-hd">📅 Calendar</div><div class="sf-export-sub">Study sessions pre-scheduled as .ics — import to Google Calendar or Outlook</div>', unsafe_allow_html=True)
        for k, v in export_meta:
            st.markdown(f'<div class="sf-export-row"><span class="sf-ek">{k}</span><span class="sf-ev">{v}</span></div>', unsafe_allow_html=True)
        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
        hpd_val = int(st.session_state.get("hpd", 2) or 2)
...
def render_tab_dag(res: dict) -> None:
    ...
    if dag_html_path.exists():
        ...
        st.warning("DAG component file not found. Ensure `components/dag_3d.html` exists.")
    ...
    # Show edge list as expandable table
...
def render_tab_interview_prep(res: dict) -> None:
    ...
    st.markdown('<div class="sf-ss">AI-generated questions per skill · based on your proficiency level · targeted to the role</div>', unsafe_allow_html=True)

    # Readiness summary
    iv_color = iv.get("color", "#4ade80")
    score    = iv.get("score", 0)
    label    = iv.get("label", "–")
    advice   = iv.get("advice", "")

    col1, col2, col3 = st.columns(3)
    col1.metric("Readiness score",    f"{score}%",               label)
    col2.metric("Skills you can answer", str(iv.get("req_known", 0)),    "ready now")
    col3.metric("Skills needing prep",   str(iv.get("req_missing", 0)), "study first")

    st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)

    # Known/Partial skills — these are the ones to prep
    ...
    if iq:
                ...
                if qs:
                    for i, q in enumerate(qs[:3], 1):
                        st.markdown(
                            ...
                            f'<div style="font-family:var(--mono);font-size:0.62rem;color:var(--teal);'
                            f'margin-bottom:5px">Q{i}</div>'
        ...
        known_skills = [g for g in gp if g["status"] == "Known" and g["is_required"]]
        if known_skills:
            for g in known_skills:
                ctx = g.get("context","")
                if ctx:
                    st.markdown(
                        f'<div class="sf-talk">→ <strong>{g["skill"]}</strong> ({g["proficiency"]}/10): {ctx}</div>',
                        unsafe_allow_html=True
                    )
    else:
        # Show skill cards even before questions generated
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        for g in preppable:
            skill  = g["skill"]
            prof   = g["proficiency"]
            status = g["status"]
            col_c  = _TEAL if status == "Known" else _AMBER
            st.markdown(
                f'<div style="background:var(--s1);border:1px solid var(--border);'
                f'border-left:3px solid {col_c};border-radius:0 9px 9px 0;'
                f'padding:12px 16px;margin-bottom:8px;">'
                f'<div style="font-size:0.88rem;font-weight:600;color:var(--t1)">{skill} · {prof}/10</div>'
                f'<div style="font-family:var(--mono);font-size:0.68rem;color:var(--t3);margin-top:4px">'
                f'{g.get("context","No context available")}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
...
def render_sidebar(res: dict) -> None:
    ...
    with st.sidebar:
        ...
        sections = [("gap", "#2dd4bf", "Gap Analysis"), ("roadmap", "#f59e0b", "Roadmap"), ("research", "#4ade80", "Research"), ("ats", "#a78bfa", "ATS & Export")]
        ...
        # API log — sidebar only, never in main content
        if _bk._audit_log:
            st.markdown('<div style="height:1px;background:rgba(255,255,255,0.07);margin:10px 0"></div>', unsafe_allow_html=True)
            ...
            for e in _bk._audit_log[-4:]:
                ...
                st.markdown(f'<div class="sf-log"><span style="color:{sc}">{"●" if ok else "✕"}</span><span>{e.get("ts","")}</span><span style="color:var(--teal)">{e.get("model","")}</span><span>{e.get("in",0)}+{e.get("out",0)}tok</span><span>${e.get("cost",0):.5f}</span></div>', unsafe_allow_html=True)
            st.markdown(f'<div style="font-family:var(--mono);font-size:0.6rem;color:var(--t4);padding:2px 12px">total: ${total_cost:.5f}</div>', unsafe_allow_html=True)
...
# =============================================================================
#  FOOTER — FIX: no ↑# anchor hack
# =============================================================================
def render_footer(res: dict) -> None:
    total_cost = sum(e.get("cost", 0) for e in _bk._audit_log)
    ...
    st.markdown(
        ...
        f'<span style="color:var(--t4)">{cache_lbl} · {len(_bk._audit_log)} calls · ${total_cost:.5f}</span>'
        f'<span class="sf-fr">SkillForge v12</span>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
def render_results() -> None:
    ...
    st.markdown('<div class="sf-page sf-page-pad">', unsafe_allow_html=True)

    # Reset button — placed in a proper right-aligned row
    ...
    tab_gap, tab_road, tab_dag, tab_prep, tab_research, tab_ats = st.tabs([
        "Gap Analysis", "Roadmap", "🔗 DAG View", "🎤 Interview Prep", "Research", "ATS & Export",
    ])
