# =============================================================================
#  main.py — SkillForge  AI-Adaptive Onboarding Engine  v2
...
Data Visualization, AWS (SageMaker)
...
def _groq(prompt: str, system: str = "You are an expert HR analyst. Always respond with valid JSON only, no markdown fences.") -> dict:
    try:
        ...
        return {"error": "JSON parse failed", "raw": raw[:200] if 'raw' in dir() else ""}
    except Exception as e:
        return {"error": str(e)}
...
# ──────────────────────────────────────────────────────────────────────────────
# SECTION 9B · READINESS ESTIMATOR
# ──────────────────────────────────────────────────────────────────────────────
...
def radar_chart(gap_profile: List[dict], dark_mode: bool = True) -> go.Figure:
    ...
    fig.update_layout(
        ...
        paper_bgcolor=_DARK_BG, plot_bgcolor=_DARK_BG, font=font, showlegend=True,
...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    body.light-mode .hero-sub{color:#4A5568 !important}
    body.light-mode .impact-lbl,.light-mode .section-s,.light-mode .upload-hint,.light-mode .logo-sub{color:#718096 !important}
    ...
    body.light-mode .mod-meta{color:#718096 !important}
    body.light-mode .warn-banner{background:rgba(255,193,7,.12) !important;color:#92400E !important}
    body.light-mode .prof-val{color:#1A202C !important}
    body.light-mode .section-h{color:#1A202C !important}
    /* NAV */
    ...
    /* LOADING OVERLAY */
    .loading-overlay{display:none;position:fixed;inset:0;background:rgba(7,11,20,.88);z-index:9999;align-items:center;justify-content:center;flex-direction:column;gap:14px}
...
<body id="body-root">
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Hide overlay whenever results store updates
      var obs = new MutationObserver(function() {
        var store = document.getElementById('s-results');
        if (store) { overlay.classList.remove('active'); }
      });
...
app.layout = html.Div([
    ...
    # Loading overlay (plain HTML, JS-controlled for reliability)
    html.Div([
    ...
    dbc.Container([
        ...
        html.Div([
            ...
            html.P("Upload your resume and target job description — the AI identifies "
                   "your skill gaps and builds a dependency-aware learning roadmap.",
                   className="hero-sub"),
        ...
        dbc.Row([
            dbc.Col([
                html.Div([
                    ...
                    dcc.Upload(
                        id="up-resume",
                        children=html.Div(["Drop or ", html.Span("browse",
                            style={"color":"#4ECDC4","textDecoration":"underline"})]),
                        className="upload-box",
                    ),
                    html.P("PDF or DOCX", className="upload-hint"),
                    html.Div(id="fn-resume", style={"fontSize":".75rem","color":"#4ECDC4",
            ...
            dbc.Col([
                html.Div([
                    ...
                    html.Div(id="fn-jd", style={"fontSize":".75rem","color":"#4ECDC4",
                                                 "marginTop":"6px","textAlign":"center"}),
                    dbc.Textarea(id="jd-paste",
                                 placeholder="…or paste the JD text here",
                                 rows=3, style={"marginTop":"10px"}),
                ], className="glass-card", style={"textAlign":"center"}),
            ], md=5),
...
# Dark/Light mode toggle — clientside for instant response
app.clientside_callback(
    """
    function(n_clicks, theme) {
        if (!n_clicks) return theme || 'dark';
...
# Hide loading overlay when results arrive — clientside
app.clientside_callback(
    """
    function(data) {
        var overlay = document.getElementById('loading-overlay');
...
# Upload handlers
@app.callback(
    Output("fn-resume", "children"),
    Output("s-resume",  "data"),
    Input("up-resume",  "contents"),
    State("up-resume",  "filename"),
    prevent_initial_call=True,
)
...
@app.callback(
    Output("fn-jd", "children"),
    Output("s-jd",  "data"),
    Input("up-jd",  "contents"),
    State("up-jd",  "filename"),
    prevent_initial_call=True,
)
def cb_jd_upload(contents, filename):
    if not contents:
        return "", None
    return f"✓ {filename}", {"text": parse_upload(contents, filename), "filename": filename}


# Sample inputs — load preset text into stores
@app.callback(
...
def cb_samples(n1, n2, n3):
    ...
    key_map = {
        "sample-junior": "junior_swe",
        "sample-senior": "senior_ds",
        "sample-hr":     "hr_manager",
    }
    ...
    s = SAMPLES[key]
    return (
        {"text": s["resume"], "filename": f"{key}.txt"},
...
def cb_run(n, resume_store, jd_store, jd_paste):
    ...
    if not jd_text_final:
        return no_update, {"display":"none"}, "⚠ Upload, paste, or load a job description."
    ...
    # Parallel reasoning generation for first 12 modules
    ...
    with ThreadPoolExecutor(max_workers=4) as ex:
        ...
        for f in as_completed(fut_map):
            idx = fut_map[f]
            try:
                subset[idx]["reasoning"] = f.result()
            except Exception:
                subset[idx]["reasoning"] = f"Addresses gap in {subset[idx]['gap_skill']}."
...
def cb_tabs(tab, theme, results):
    ...
    jd   = results["jd_data"]
    gp   = results["gap_profile"]
    pt   = results["path"]
    im   = results["impact"]
    sm   = results.get("seniority", {})

    # Seniority warning
    warn_banner = None
    if sm.get("has_mismatch"):
        warn_banner = html.Div([
            html.Span("⚠ Seniority Gap: ", style={"fontWeight":"700"}),
            f"Candidate is {sm['candidate']}, role requires {sm['required']}. "
            "Leadership modules auto-added to roadmap.",
    ...
    if tab == "tab-gap":
        ...
        def skill_row(g):
            ...
            decay = (html.Span("↓ decayed", className="decay-badge ms-1")
                     if g.get("decayed") else None)
            items = [
            ...
            dbc.Row([
                dbc.Col([html.Div([
                    ...
                    html.P(f"{rd.get('name','Candidate')}  vs  {jd.get('role_title','Target Role')}",
                           className="section-s"),
                    dcc.Graph(figure=radar_chart(gp, dark),
                ...
                dbc.Col([html.Div([
                    ...
                    html.P(f"{len(known)} Known · {len(partial)} Partial · {len(missing)} Missing"
                           + (f" · {len(decayed)} decay-adjusted" if decayed else ""),
                           className="section-s"),
                ...
                dbc.Col([html.Div([
                    ...
                    *[html.Div([html.Span(k, className="prof-key"),
                                html.Span(str(v), className="prof-val")],
                               className="prof-row")
                ...
                dbc.Col([html.Div([
                    ...
                    *[html.Div([
                      ...
                      for n, col, lbl in [
                          (len(known),   "#4ECDC4", "Known"),
                          (len(partial), "#FFE66D", "Partial"),
                          (len(missing), "#FF6B6B", "Missing"),
                          (len(decayed), "#FFA726", "Decay-adjusted"),
                      ]],
                ], className="glass-card")], md=4),
            ], className="g-3"),
        ])
    ...
    if tab == "tab-road":
        ...
        def mod_card(i, m):
            ...
            xtra  = (" critical" if is_cr
                     ...
                     else " int" if m["level"]=="Intermediate"
                     else "")
            meta_items = [
                html.Span(f"Skill: {m['skill']}  ·  Gap: {m.get('gap_status','—')}",
                          className="mod-meta"),
                domain_badge(m["domain"]),
            ]
            if is_cr:
                meta_items.append(html.Span("⚡ critical path",
                                            className="critical-badge ms-1"))
            return html.Div([
                html.Div([
                    html.Span(f"#{i+1}",
        ...
        fit_card = html.Div([
            ...
            dbc.Row([
                dbc.Col([html.Div([
                    html.Div(f"{im['current_fit']}",
                             className="fit-num-big", style={"color":"#FF6B6B"}),
                ...
                dbc.Col([html.Div("→", style={"fontSize":"2rem","color":"#3D4F6B",
                        ...
                        width="auto"),
                dbc.Col([html.Div([
                    html.Div(f"{im['projected_fit']}",
                             className="fit-num-big", style={"color":"#4ECDC4"}),
        ...
        return html.Div([
            ...
            dbc.Row([
                ...
                dbc.Col([html.Div([
                    ...
                    dbc.Row([
                        dbc.Col([html.Div(f"~{im['hours_saved']}h", className="impact-num"),
                                 html.Div("Hours Saved", className="impact-lbl")],
                                className="text-center"),
                        dbc.Col([html.Div(f"{im['roadmap_hours']}h", className="impact-num"),
                                 html.Div("Training Time", className="impact-lbl")],
                                className="text-center"),
                        dbc.Col([html.Div(str(im["modules_count"]), className="impact-num"),
                                 html.Div("Modules", className="impact-lbl")],
                                className="text-center"),
                        dbc.Col([html.Div(str(im.get("critical_count",0)),
                                          className="impact-num", style={"color":"#FF6B6B"}),
                                 html.Div("Critical Path", className="impact-lbl")],
                                className="text-center"),
                    ], className="g-2"),
                    html.Div(style={"height":"14px"}),
                    html.Div("Skill Coverage",
                             style={"fontSize":".72rem","color":"#3D4F6B","marginBottom":"5px"}),
                    html.Div(className="prog-track", children=[
                        html.Div(className="prog-fill",
                                 style={"width":f"{im['projected_fit']}%"}),
                    ]),
                    html.Div(style={"height":"14px"}),
                    # Readiness estimator
                    html.Div([
                        html.Span("Study pace: ",
                                  style={"fontSize":".8rem","color":"#3D4F6B","marginRight":"8px"}),
                        dcc.Dropdown(
                            id="pace-dd",
                        ...
                        html.Span(
                            f"Ready in ~{weeks_to_ready(im['roadmap_hours'], 2)}",
                            id="ready-estimate",
                ...
                dbc.Col([html.Div([
                    ...
                    html.P(f"{im['modules_count']} modules · {im['roadmap_hours']}h total · "
                           f"{im.get('critical_count',0)} critical", className="section-s"),
                    dcc.Graph(figure=timeline_chart(pt, dark),
    ...
    if tab == "tab-rep":
        checklist = [
            ...
            "Parallel Reasoning via ThreadPoolExecutor",
...
# Readiness estimator — updates when pace dropdown changes
@app.callback(
...
def cb_ready(pace, results):
    if not results or not pace:
        raise PreventUpdate
    hrs = results["impact"]["roadmap_hours"]
    return f"Ready in ~{weeks_to_ready(hrs, pace)}"
...
if __name__ == "__main__":
    ...
    print(f"  → Semantic matching : {'sentence-transformers ✓' if SEMANTIC else 'substring fallback (install sentence-transformers for better matching)'}")
    print(f"  → PDF export        : {'reportlab ✓' if REPORTLAB else 'not available'}")
    print(f"  → Groq API key      : set ✓")
    print("  → New features      : Skill Decay · Role Fit Score · Critical Path")
